...
bool PlayerPhysics::movement(GLFWwindow* window,float dt){
	...
	glm::vec4 pos = trans*glm::vec4(0.0f, -0.9f, 0.0f,1.0f);
	bool ok = false;

	if (glfwGetKey(window, GLFW_KEY_LEFT)&& pos.x>=-1.0f){
		dir.x += -dt*speed;
	...
	if (glfwGetKey(window, GLFW_KEY_RIGHT)&& pos.x<=1.0f){
		dir.x += dt*speed;
...
glm::mat4 PlayerPhysics::getTrans(){
	return trans;
}